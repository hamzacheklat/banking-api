Parfait ! Je vais te donner une **version complète prête à copier** du projet **Backend + Frontend** mis à jour pour **LiteLLM uniquement** (embeddings + LLM), sans clé OpenAI.

---

# ✅ Backend — FastAPI + LiteLLM + Chroma
...
## 3️⃣ `awr_oracle.py` — Oracle AWR extraction

*(Même code que précédemment, inchangé, gère snapshots, top SQLs, plans)*

---

## 4️⃣ `rag_chroma.py` — LiteLLM pour embeddings + LLM
...
def _retrieve_context(text: str, top_k: int = 5) -> List[str]:
    """Return top_k retrieved documents (strings) from Chroma."""
    try:
...
def enrich_with_rag_and_llm(prompt_body: str, top_k: int = 5) -> str:
    ...
    try:
        return llm(prompt, temperature=0.15)
    except Exception as e:
        return f"Error calling LiteLLM: {e}"
```
...
app = FastAPI(title="Oracle AWR + LiteLLM + AI Tuning Advisor")
...
class TuningReq(DoubleIntervalReq):
    top_n_sql: int = 5
...
export class AppComponent {
  ...
  analyze() {
    ...
    this.api.analyzeIntervals({
      ...
      next: (res) => { this.analysisResult = res.analysis; this.loading = false; },
      error: (err) => { this.analysisResult = JSON.stringify(err); this.loading = false; }
    });
  }
  ...
  runAdvisor() {
    ...
    this.api.tuningAdvisor({
      ...
      next: (res) => { this.topSqls = res.top_sqls || []; this.advisorResult = res.advisor_report; this.loading = false; },
      error: (err) => { this.advisorResult = JSON.stringify(err); this.loading = false; }
    });
  }
}
```
...
<div style="max-width:1000px; margin:20px auto; padding:20px; border:1px solid #ddd; border-radius:8px;">
  <h2>Oracle AWR Analyzer & LiteLLM Tuning Advisor</h2>
  ...
  <div style="margin-top:12px;">
    ...
    <button (click)="runAdvisor()" [disabled]="loading" style="margin-left:8px;">LiteLLM Tuning Advisor</button>
  ...
  <div *ngIf="advisorResult" style="margin-top:18px;">
    <h3>LiteLLM Tuning Advisor Report</h3>
...
CORS permet `http://localhost:4200` par défaut.

---

Cette version **n’a plus besoin de clé OpenAI**, utilise LiteLLM **pour les embeddings et LLM**, et le frontend Angular fonctionne directement.

---

Si tu veux, je peux te faire un **script pour indexer automatiquement ton `rag_corpus` avec LiteLLM** prêt à l’emploi.
